<template>
  <div class="group"  :class="[`nodeType-${el.$$nodeType}`,{ actived: el.$$actived,overed:el.$$overed,fold:el.$$fold}]" v-for="(el,index) in elList" 
   ...
   <span v-if="el.children">-</span>
    ...
    /></span>
    <span class="tagEnd" v-show="el.$$nodeType!=3">&lt;/{{ el.tagName }}></span>
  </div>
</template>

<style>
#ideDesigner,#ideDesigner span{
    font-family:'Microsoft YaHei','Courier New', Courier, monospace;font-size: 13px;
}
#ideDesigner div{
   
}
#ideDesigner .group{
    margin-left:20px;border:solid 1px white;
}
...
#ideDesigner .group.actived{
  border:solid 1px blue;background-color:ivory;
}
#ideDesigner .group.fold{
    height:24px;overflow-y:hidden;
}
#ideDesigner .line{
  display: inline-block; cursor:default;border:solid 1px white;height: 20px;margin-top:2px;
}
...
#ideDesigner .prop{
    border: 0px solid rgb(240, 237, 237);margin-left:10px;
    position: relative;
}
...
#ideDesigner input{
    display: inline-block;position: absolute;width:100% ;border:1px solid white;left:0;outline:none;line-height: 16px; height:16px;
}

#ideDesigner input:hover{
    color:red;border:1px solid rgb(236, 227, 227);
}
#ideDesigner input:focus{
    color:red;border:1px solid rgb(167, 159, 159);width:100%;
}
#ideDesigner .tagEnd{
    margin-left: 4px;
}
